...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _dsoc is not None:
        ...
        _хвост = ("" if (_публично and not is_owner(update)) else
                  ("<blockquote expandable>"
                   + подпись.strip().replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
                   + "</blockquote>"))
        # Владелец и в чате хозяин своим цифрам — но и ему в общем чате они ни к чему: он
        # видит их в личке и в журнале. В группе не показываем НИКОМУ, включая его.
        if _публично:
            _хвост = ""
        _тело = _тело + _кто
        try:
