        ...
        try:
            ...
            try:
                ...
                if isinstance(queue, list) and queue:
                    ...
                    # ⛔ И СРАЗУ ОГРАДА К ЭТОЙ ЖЕ ПРАВКЕ (поймано через 3 минуты после выкатки).
                    # Номера постов пишутся недавно: в app_post_msgids два десятка записей, а в
                    # app_post_ids — сотни. Буквальное «нет номера → не выходило» объявило
                    # брошенными ВСЕ старые версии и полезло переиздавать v860. Владельцу в
                    # канал посыпалась бы история за месяцы.
                    # ПРАВИЛО: брошенной считаем только версию НОВЕЕ той, с которой мы вообще
                    # начали записывать номера. Всё, что старше, — не «не вышло», а «мы тогда
                    # не считали». Отсутствие доказательства не есть доказательство отсутствия.
                    _мид = j.get("app_post_msgids") or {}
                    def _номер(_v):
                        try:
                            return int("".join(c for c in str(_v) if c.isdigit()) or 0)
                        except Exception:
                            return 0
                    # ⛔⛔ ОГРАДА СЛОМАЛАСЬ О СВОЙ ЖЕ ЛИБЕРАЛИЗМ (поймано за минуту, владелец:
                    # «у меня в муслимун боте спам!!!»). Порог считался по ВСЕМ ключам номеров,
                    # а среди них лежит «dsoc-1» — из него _номер вытащил единицу, порог стал 1,
                    # и все версии начиная с v853 прошли как «брошенные». В канал полетела
                    # история за месяцы.
                    # Берём ТОЛЬКО настоящие версии — строго «v» и цифры. Разбирать как число
                    # всё, что попалось, — это и есть та самая доверчивость, которая уже раз
                    # обошлась нам ключами из первого слова ноты.
                    _вер_ключи = [k for k in _мид if re.fullmatch(r'v\d{2,6}', str(k) or '')]
                    _порог = min([_номер(k) for k in _вер_ключи] or [10 ** 9])
                    _брошено = [x for x in posted_ids
