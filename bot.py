    ...
    async def errlog(r):
        ...
        try:
            ...
            if existing:
                ...
                # 26.07.2026, владелец в гневе: «почему он пишет мне хрень... почему ошибки мост тебе
                # напрямую не приходит?» Он прав. Заявка #250 просила уведомлять его — но тогда ошибок
                # были единицы, а теперь 129 из 138 это ОДНА И ТА ЖЕ ложная тревога сверки по Муслиму,
                # и она превратила его личку в свалку. Разбирать ошибки — моя работа, не его: я читаю
                # errors.json напрямую из ветки data, Telegram для этого не нужен.
                # Владельцу шлём ТОЛЬКО то, что касается его лично: поломку прода, а не рабочий шум.
                _важно = any(с in str(where or '') for с in ('bot', 'backend', 'прод', 'fetchJSON'))
                try:
                    if application and _важно:
                        await application.bot.send_message(OWNER_ID, _enote)
                    elif application:
                        await application.bot.send_message(LOG_CHAT_ID, _enote)
                except Exception: pass
