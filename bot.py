...
def gemini_вызов(тело, timeout=90):
    ...
    for мод in порядок:
        try:
            ...
            if о.status_code == 200:
                j = о.json()
                т = (((j.get("candidates") or [{}])[0].get("content") or {})
                     .get("parts") or [{}])[0].get("text", "").strip()
                if т:
                    return т, "", мод
                беды.append("%s: ответ пустой" % мод)
                continue
...
async def dsoc_инструмент(строка, бот=None, чат=None):
    ...
    try:
        ...
        if м:
            ...
            _файлы = [('bot.py', 'бот и его команды'),
                      ('miniapp/index.html', 'мини-приложение: кнопки, экраны, карточки'),
                      ('dsoc_promt.md', 'твой собственный системный промт')]
            _наш = []
            try:
                _обр = re.compile(re.escape(_что), re.I)
            except Exception:
                return 'Не понял, что искать в коде.'
            for _имя, _оп in _файлы:
                try:
                    _текст = open(_имя, encoding='utf-8-sig', errors='ignore').read()
                except Exception:
                    continue
                ...
                if len(_наш) >= 6:
                    break
            if not _наш:
                return ('В коде «%s» не нашлось. Ищи по тому, что видно на экране: надпись на '
                        'кнопке, текст ошибки, имя поля. По смыслу («где кнопка перевода») код '
                        'не ищется — он ищется по буквам.') % _что
        ...
        if м:
            ...
            if _код:
                ...
                try:
                    ...
                    _ар = _ру = ""
                if _ар or _ру:
                    return ("У автора (аль-Мухаймин) метки «%s %s» НЕТ — он этот хадис не "
                            "приводил. Но САМ ПЕРВОИСТОЧНИК у нас есть, вот он. Так и скажи "
                            "владельцу: «у Муршида не встречается, в первоисточнике есть».\n\n"
                            "%s №%s\n\n%s%s"
                            % (_сб, _ном, NAMES.get(_к2, _к2), _ном, (_ар or ''),
                               (chr(10) + chr(10) + _ру) if _ру else ''))
...
def dsoc_промт_контекстные():
    ...
    try:
        д = dsoc_свод_правил()
        поз = д.get("позиции") or []
        готовые = [п for п in поз if п.get("статус") == "контекст"]
...
def _нейро_сборник(имя):
    ...
    return NEURO_ИМЕНА.get(н) or NEURO_ИМЕНА.get((имя or "").lower().strip())
...
async def dsoc_уши(биты, имя):
    ...
    if _р not in DSOC_СЛЫШИМЫЕ:
        return "(«%s» — такой звук я разбирать не умею)" % имя
    if not OPENAI_API_KEY:
        return "(запись «%s» пришла, но расшифровать нечем — Whisper недоступен)" % имя
    ...
    try:
        ...
        if not _т:
            return "(запись «%s» не расшифровалась — речи не разобрать)" % имя
        _видео = _р in ('.mp4', '.webm', '.mpeg')
        return ("=== %s «%s» ===" % ("ЧТО СЛЫШНО В ВИДЕО (картинку не вижу, только звук)"
                                     if _видео else "ЧТО СКАЗАНО В ЗАПИСИ", имя)
                + chr(10) + _т)
    except Exception as e:
...
def dsoc_системный_запасной():
    ...
    return (
        ...
        "ВЫЗОВ: документ <имя> || <содержимое> — СОБРАТЬ файл из своего текста и отдать его "
        "человеку. Просят инструкцию, памятку, список, выжимку, «пришли файлом», «сделай md» — "
        "пиши содержимое сам и отдавай этим вызовом. Это не пересказ в чат: человек получает "
        "вещь, которую можно сохранить и переслать. Длинные разборы тоже лучше файлом, чем "
        "простынёй в чате.\n"
        "ВЫЗОВ: слово <арабское слово> [сборник] — ЕСТЬ ЛИ такое слово в самих первоисточниках. "
        "Ищет по тексту со снятыми огласовками, поэтому «مذهب» находит и там, где написано "
        "«مَذْهَب». Спрашивают «есть ли в Абу Дауде хадис со словом X», «встречается ли такое "
        "слово» — БЕРИ ЭТОТ ВЫЗОВ, а не «поиск»: тот шарит только по книге аль-Мухаймина и на "
        "такой вопрос ответит «нет» о том, что есть.\n"
        "ВЫЗОВ: смс <номер> [окно]   — ДОСТАТЬ сообщения этого чата по номеру, окном ±N "
        "(до 25). Просят вникнуть в диалог, разобрать последние сообщения, найти, кто что "
        "сказал, дали ссылку на сообщение — БЕРИ ЭТОТ ВЫЗОВ. Номер сообщения — последнее "
        "число в ссылке вида t.me/…/726201. Говорить «полного лога у меня нет, пришли ещё раз» "
        "БОЛЬШЕ НЕЛЬЗЯ: ты умеешь читать сам, а просить человека повторить то, что можешь "
        "прочитать, — перекладывать свою работу на него.\n"
        "ВЫЗОВ: аят <сура>:<аят>     — аят Корана: арабский оригинал + перевод Кулиева. "
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if _хозяин(update) and update.message.reply_to_message:
            ...
            if _мой_ли_ответ:
                ...
                if _его_дело and not _ко_мне_явно:
                    _мой_ли_ответ = False
    except Exception:
        _мой_ли_ответ = False
    ...
    if _dsoc is not None:
        ...
        for _круг in range(3):
            ...
            try:
                await живое.edit_text(
                    "%s… %d с%s" % (_чем_занят(_вз.group(1)), int(time.time() - _старт_дсок),
                                    ('  ·  заход %d из 3' % (_круг + 1)) if _круг else ''))
            except Exception:
                pass
            данные = await dsoc_инструмент(_вз.group(1).strip(), бот=context.bot, чат=chat_id)
            if not данные:
                break
            ...
            if isinstance(данные, str) and данные.startswith('КАК_ЕСТЬ|'):
                ...
                try:
                    await живое.edit_text(_готово + _кто_к, parse_mode="HTML",
                                          disable_web_page_preview=True)
                    if _матн_п:
                        _пер = ''
                        try:
        ...
        if re.search(r'^\s*ВЫЗОВ:', собрано or '', re.M):
            ...
            _невышел = (re.search(r'^\s*ВЫЗОВ:\s*(\S+)', собрано, re.M) or [None, ''])[1].lower()
        ...
        try:
            ...
            if _мп:
                ...
                _записал_правило = dsoc_правило_записать(_пр.strip(), _поч, статус='ждёт',
                                                         кто='помощник')
                ...
                if _записал_правило:
                    ...
                    собрано += ("\n\n📌 **ПРАВИЛО #%d записано** · **ДЕЙСТВУЕТ с этой минуты**, "
                                "утверждения не ждёт.\nТехнадзор проверит и либо закрепит навсегда, "
                                "либо отменит — я скажу вам чем кончилось."
                                % _записал_правило)
                    ...
                    try:
                        await application.bot.send_message(
                            ...
                            % (_записал_правило, (_пр.strip())[:900]
                               .replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')),
                            parse_mode='HTML')
                    except Exception:
                        pass
        ...
        try:
            ...
            elif DSOC_ОТКАЗ_РЕ.search(_низ_отв):
                _прич = 'отказ: ' + DSOC_ОТКАЗ_РЕ.search(_низ_отв).group(0)
            if _прич:
                ...
                try:
                    ...
                    if _нз:
                        собрано = (собрано or '') + (
                            ...
                            'передал технадзору заявкой #%s, чтобы научился. '
                            'Он ответит здесь же.' % _нз)
                except Exception:
                    pass
        except Exception:
            pass
        ...
        if предложение:
            ...
            if _зн:
                ...
                # ТОТ, кто его выводит.
                подпись = ("\n📥 Записал заявкой технадзору — обращение #%s: «%s»\n"
    ...
    if user_id != OWNER_ID and text and not _ai_loop_guard(update, text) and not _AI_PUBLIC_OFF:  # 🔒 мастер-рубильник: ИИ не-владельцу ВЫКЛ
        ...
        if _triggered and feature_allowed('bot', tg_user_dict(update)):
            ...
            if _nq:
                ...
                await send_long(update, result, parse_mode="HTML")
    ...
    if collection:
        ...
        if number:
            ...
            try:
                ...
                _пост_х = None
                try:
                    await _ждём_х.edit_text(_тело_х[:3900], parse_mode="HTML",
                                            disable_web_page_preview=True)
                    _пост_х = _ждём_х
                except Exception:
                    try:
                ...
                if _переведём_потом and _пост_х is not None:
                    ...
                    try:
                        ...
                        _ru = ""
                    _готовое = (_тело_х.replace(_хвост_п, "")
                                + (("\n\n🌍 " + esc(_ru)) if _ru
                                   else "\n\n<i>Перевод не собрался — показываю только арабский.</i>"))
                    try:
                        await _пост_х.edit_text(_готовое[:3900], parse_mode="HTML",
                                                disable_web_page_preview=True)
                    except Exception:
                        if _ru:
                            try:
                                await update.message.reply_text("🌍 " + _ru[:3800])
                            except Exception:
                                pass
                return
...
async def _api_serve(application=None):
    ...
    async def ochered(r):
        ...
        if body.get('все'):
            return _cors(web.json_response({'ok': True, 'счёт': счёт, 'все': сп}))
        return _cors(web.json_response({'ok': True, 'счёт': счёт,
    ...
    async def skazat(r):
        ...
        _МЕТКА = '🧠 <b>Клод (технадзор)</b>'
        if not body.get('без_подписи') and _МЕТКА not in текст and '🧠 Клод' not in текст:
            текст = _МЕТКА + chr(10) + chr(10) + текст
        ...
        try:
            ...
            try:
                ...
                if _чид is not None:
                    ...
                    _пам.append({'role': 'assistant',
                                 'content': '[ТЕХНАДЗОР СКАЗАЛ В ЭТОМ ЧАТЕ] '
                                            + re.sub(r'<[^>]+>', '', текст)[:3000],
                                 't': time.time()})
