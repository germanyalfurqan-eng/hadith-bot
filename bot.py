    ...
    if is_owner(update):
        ...
        if _tl.startswith("заявка ") or _tl.startswith("замечание ") or _tl == "заявка" or _tl == "замечание":
            ...
            if rep:
                ...
                # 25.07.2026 (владелец показал «Заявка #637» = текст хадиса Сахих Муслим №7): ответ «заявка»
                # БЕЗ своего комментария на сообщение БОТА регистрировал текст самого бота — хадис, сводку про
                # API, «#клод_сказал». В реестре 16 таких пустышек из 647. Полезный случай (ответ + комментарий)
                # не трогаем: там ясно, что владелец хочет сделать. А вот молчаливый ответ на свой же пост —
                # это не заявка, а промах пальцем. Просим одну строку смысла вместо мусора в журнале.
                _rep_is_bot = bool(getattr(getattr(rep, "from_user", None), "is_bot", False))
                if _rep_is_bot and not body:
                    await update.message.reply_text(
                        "✍️ Это моё же сообщение, а своего текста ты не добавил — заявки не получится.\n"
                        "Напиши одной строкой, что с этим сделать: *заявка <что нужно>* ответом на то же сообщение.",
                        parse_mode="Markdown")
                    return
                if rep_text:
