    ...
    async def neuro(r):
        ...
        try:
            ...
            sysm = ("Ты — умный поиск по Корану и хадисам. Запрос на русском (ВОЗМОЖНЫ ОПЕЧАТКИ) описывает аят/хадис "
                    ...
                    "ФРАЗЫ: <1-4 УНИКАЛЬНЫЕ арабские фразы из самого текста (НЕ общие слова النبي/رسول الله!), по которым находится ИМЕННО он; через ;>\n"
                    ...
                    "ЕСЛИ узнал конкретный хадис — ОБЯЗАТЕЛЬНО дай источник и номер в ХАДИСЫ (это надёжнее фраз). "
                    "Бери РЕАЛЬНЫЕ слова текста ДОСЛОВНО как в сборнике (не пересказ, не выдумывай фразу). Если это просто тема одним словом — дай 3-5 арабских "
