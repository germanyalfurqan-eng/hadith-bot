        ...
        if number:
            ...
            try:
                ...
                if (not tr or lang == "англ") and ar:   # БАГ (владелец «почему английский»): нет русского издания → бот показывал АНГЛИЙСКИЙ. Проект русскоязычный → ИИ-перевод на русский; английский НЕ показываем
                    # В ОТДЕЛЬНОМ ПОТОКЕ И ПО ЧАСАМ. translate_matn — синхронная и небыстрая:
                    # длинный матн она гонит абзацами, десятком запросов к ИИ. Прямой вызов здесь
                    # замораживал ВЕСЬ бот на всё это время (#63: 4703 символа Ибн Маджи — ответа
                    # не было вовсе, осталась заглушка). Ждём 45 секунд: не успел — отдаём арабский
                    # и честно говорим про перевод. Хадис без перевода сейчас лучше перевода,
                    # которого никто не дождался.
                    _ru = ""
                    try:
                        _ru = await asyncio.wait_for(
                            asyncio.get_event_loop().run_in_executor(
                                None, lambda: translate_matn(ar[:6000], "had_" + str(collection),
                                                             owner=is_owner(update))), timeout=45)
                    except asyncio.TimeoutError:
                        gr = (str(gr) + " · " if gr else "") + "перевод не поспел за 45 с, показываю арабский"
                    except Exception:
                        _ru = ""
                    if _ru:
                        tr = _ru; lang = "рус"
                ...
                # Заглушка «⏳ Ищу хадис...» отслужила — снимаем перед выдачей, чтобы она не висела
                # над ответом. Не вышло снять (её удалили, прошло 48 часов) — не беда, ответ важнее.
                try:
                    await _ждём_х.delete()
                except Exception:
                    pass
