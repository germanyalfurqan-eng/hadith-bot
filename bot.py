...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _dsoc is not None:
        ...
        # ── команды самой сессии. Владелец: «ту же команду сжать контекст или новую сессию
        # открыть он тоже должен знать и подсказывать, если поймёт, что моя воля на это направлена».
        # Ловим их СЛОВАМИ, а не жёсткой формой: человек говорит «начнём заново», «сотри всё»,
        # «много набралось» — и это одно и то же желание.
        _низ = _dsoc.lower().strip()
        if any(k in _низ for k in ("новая сессия", "начнём заново", "начнем заново", "с нуля", "сброс сессии")):
            DSOC_ПАМЯТЬ[chat_id] = []
            dsoc_сохранить()
            await update.message.reply_text("🟩 Начали заново. Прежний разговор сохранён в файле сессий.")
            return
        if any(k in _низ for k in ("сожми", "ужми", "много набралось", "сократи контекст")):
            DSOC_ПАМЯТЬ[chat_id] = dsoc_ужать(dsoc_память(chat_id))
            dsoc_сохранить()
            await update.message.reply_text(
                "🟩 Ужал: %d токенов из миллиона. Начало и последние реплики целы, свёрнута середина."
                % dsoc_размер(DSOC_ПАМЯТЬ[chat_id]))
            return
        if any(k in _низ for k in ("сессия", "сколько набрали", "статус")) and len(_низ) < 30:
            р = dsoc_память(chat_id)
            await update.message.reply_text(
                "🟩 Сессия: %d реплик, %d из 1 000 000 токенов (%.1f%%)
        📊 %s"
                % (len(р), dsoc_размер(р), dsoc_размер(р) / DSOC_ОКНО * 100, dsoc_остаток_строкой()))
            return
        if any(k in _низ for k in ("что умеешь", "что ты умеешь", "команды", "помощь")):
            await update.message.reply_text(
                "🟩 Я разбираю, чего ты хочешь, и подбираю нужное сам — команды помнить не надо.
        
        "
                "• ответом на ссылку YouTube: «DSOC перескажи» / «кратко» / любой вопрос по ролику
        "
                "• ответом на любое сообщение: «DSOC переведи»
        "
                "• просто вопрос — отвечу сам
        
        "
                "Про сессию: «сколько набрали» · «сожми» · «начнём заново».")
            return
        
        # ── ролик по ссылке: субтитры идут в дело вместе с вопросом ──
        _видео_текст = ""
        try:
            _r = update.message.reply_to_message
            _истч = (getattr(_r, "text", None) or getattr(_r, "caption", None) or "") if _r else ""
            _vid = _yt_id(_истч) or _yt_id(_dsoc)
            if _vid:
                _тр = await asyncio.get_event_loop().run_in_executor(None, _yt_transcript, _vid)
                if _тр:
                    _видео_текст = ("
        
        [Расшифровка ролика — отвечай ПО НЕЙ, не по памяти; "
                                    "где уместно, ставь таймкоды]:
        " + _тр[:120000])
                else:
                    await update.message.reply_text(
                        "🎬 У этого ролика нет субтитров — пересказывать не по чему.")
                    return
        except Exception:
            pass
        ...
        реплики.append({"role": "user", "content": dsoc_обезличить(_dsoc + цитата + _видео_текст)})
