...
def _тахкик_цитатой(код_сборника, номер):
    ...
    try:
        ...
        for _бид in _биды:
            ...
            for _сл in (_в.get('w') or [])[:5]:
                _стр.append('• слабое звено: %s — %s' % (esc(str(_сл[2] if len(_сл) > 2 else '?')),
                                                         esc(str(_сл[1] if len(_сл) > 1 else '?'))))
            for _тд in (_в.get('td') or [])[:5]:
                _стр.append('• тадлис: %s передал через «ан» от %s'
                            % (esc(str(_тд[2] if len(_тд) > 2 else '?')),
                               esc(str(_тд[3] if len(_тд) > 3 else '?'))))
            _стр.append('')
            _стр.append('⚠️ Бета, и разбор ТОЛЬКО цепи: слабость цепи не всегда означает '
                        'слабость хадиса — он может быть усилен другими путями.')
