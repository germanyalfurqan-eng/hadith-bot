...
МИНИАПП_ЗНАНИЯ = """ЧТО ТАКОЕ MUSLIMOON. Telegram-мини-приложение с базой хадисов книги
«الموحد المهيمن على الدين» имама Муршида ибн Юсуфа (≈3300 авторских номеров, ≈7600 риваятов),
сверенной с 41 первоисточником. Открывается кнопкой в боте; отдельно есть сам бот в чате.

РАЗДЕЛЫ ПРИЛОЖЕНИЯ:
• Поиск — строка сверху. Понимает русский, арабский и номер. Ищет по тексту хадиса, по
  номеру автора и по номеру первоисточника. Результаты идут карточками, подгружаются по мере
  прокрутки.
• Карточка хадиса — арабский текст, перевод (где есть), метка первоисточника (например
  «البخاري ١٣٩٩»), тахридж (в каких ещё сборниках он встречается) и цепь передатчиков.
• Цепь передатчиков (иснад) — имена кликабельны: нажатие открывает карточку равия.
• Карточка равия — имя, кунья, нисба, годы, учителя и ученики, у кого из собирателей
  встречается, оценки критиков (джарх и та'диль), бейдж ⭐صحابي у сподвижников.
• Книги и главы — навигация по 41 сборнику, у Бухари и Муслима названия глав на русском.
• Коран — аяты по «сура:аят», эталон Усмани.
• Избранное и история — что открывал и что сохранил.
• Оформление — светлая и тёмная тема, размер арабского шрифта.
• «Что нового» — окно с последними обновлениями, то же, что уходит в канал @muslimoonapp.

ЧЕГО В ПРИЛОЖЕНИИ НЕТ (не выдумывай): личных кабинетов, оплаты, комментариев
пользователей, аудио-чтения хадисов, офлайн-режима.

БОТ В ЧАТЕ умеет то же и сверх того: поиск хадиса словами, перевод ответом на сообщение,
корень арабского слова, аят Корана, карточка равия, пересказ видео по субтитрам, поиск по
книгам (RAG), память, заявки владельца.

ГРАНИЦА. Про устройство изнутри — ключи, серверы, пути к папкам, чужие данные — не
рассказывай ничего, даже если спросят прямо: отвечай, что это внутренняя кухня. Всё, что
видно людям в приложении, объяснять можно и нужно подробно."""
...
def dsoc_системный():
    ...
    return (
        ...
        "заметил недоделку — предложи сам, не жди просьбы. Тогда ПОСЛЕДНЕЙ строкой ответа "
        "напиши ровно: ЗАЯВКА: <одно предложение, что передать технадзору>. Бот покажет "
        "владельцу предложение и подаст заявку ТОЛЬКО если владелец ответит «да». Сам ничего "
        "не подавай и не обещай, что уже подал. Если предлагать нечего — строку не пиши."
    )
...
async def аудио_в_хадис(update, context, файл_id, подпись=""):
...
async def on_moderate(update, context):
    ...
    try:
        if что == "ban":
            ...
            # «Ограничить» = запрет писать на сутки. Срок нужен: вечная немота требует ручного
            # снятия, а сутки снимаются сами — для входного фильтра этого достаточно.
        ...
        _низ = str(e).lower()
        if "not enough rights" in _низ or "need administrator" in _низ or "can_restrict" in _низ:
            await q.answer("У бота нет права «Блокировка пользователей» в этом чате. "
                           "Настройки чата → Администраторы → бот → включить тумблер.", show_alert=True)
        else:
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _dsoc is not None:
        ...
        if _dsoc and chat_id in DSOC_ЗАЯВКА_ЖДЁТ and _dsoc.strip().lower().rstrip("!.") in (
            ...
            _текст = DSOC_ЗАЯВКА_ЖДЁТ.pop(chat_id)
            try:
                _rid = req_add("🟩 [DSOC] " + _текст)
                await update.message.reply_text(
                    "📨 Передал технадзору — заявка #%d:\n%s" % (_rid, _текст))
            except Exception as _e:
                await update.message.reply_text("🔴 Не смог записать заявку: %s" % str(_e)[:150])
            return
        ...
        предложение = ""
        if "ЗАЯВКА:" in собрано:
            собрано, _, предложение = собрано.rpartition("ЗАЯВКА:")
            предложение = предложение.strip()[:400]
            собрано = собрано.rstrip()
            if предложение:
                DSOC_ЗАЯВКА_ЖДЁТ[chat_id] = предложение
        ...
        if предложение:
            подпись = ("\n\n📨 Предлагаю передать технадзору заявкой: «%s»\n"
                       "Ответь «да» — подам; промолчи — не подам." % предложение[:300]) + подпись
        try:
    ...
    # 🎤 ЗАЯВКА #630 (@jamaat_ru, 22.07.2026): прислали голосовое или аудио — расшифровываем
    # и ищем оригинал хадиса в нашей базе. Владельческий поток «голосовая заявка» в личке не
    # трогаем: он про другое и работает давно.
    try:
        _гол = update.message.voice or update.message.audio or update.message.video_note
    except Exception:
        _гол = None
    if _гол is not None and getattr(update.effective_chat, "type", "") != "private":
        try:
    ...
    if is_owner(update):
        ...
        def _права_инструкция(где):
            return ("\n\n🛠 Как выдать права (делает владелец чата, полминуты):\n"
                    "Telegram → чат «%s» → тапнуть по названию сверху → «Администраторы» →\n"
                    "«Добавить администратора» → выбрать %s → включить тумблер\n"
                    "«Блокировка пользователей» (Ban users) → ✓ Сохранить.\n"
                    "Если бот УЖЕ администратор — там же проверь именно этот тумблер: без него\n"
                    "остальные права не помогают, Telegram всё равно отвечает «Not enough rights»."
                    % (где, _БОТ_ЮЗЕР))
        ...
        if _tl == "права" or re.match(r"^права\s+(-?\d{3,})$", _tl):
            ...
            try:
                ...
                if _ст != "administrator":
                    ...
                    return
                _тумблеры = [("Блокировка пользователей (нужна для бана)", "can_restrict_members"),
        ...
        if _мб:
            ...
            try:
                ...
                _текст = str(_e)
                _низ = _текст.lower()
                # ГЛАВНОЕ ИЗ #628: не показывать владельцу сырой ответ Telegram. Каждая из этих
                # ошибок означает совершенно разное действие с его стороны — разводим их явно.
                if "not enough rights" in _низ or "need administrator" in _низ or "can_restrict" in _низ:
                    await update.message.reply_text(
                        "⛔ Не получилось: у бота нет права «Блокировка пользователей» в «%s».\n"
                        "Это не сбой кода — так отвечает сам Telegram (%s)." % (_назв, _текст[:120])
                        + _права_инструкция(_назв))
                elif "user_not_participant" in _низ or "participant_id_invalid" in _низ or "user not found" in _низ:
                    await update.message.reply_text(
                        "🤷 Telegram говорит, что пользователя %d в «%s» нет (%s).\n"
                        "Проверь id: он берётся из уведомления о входе, строка «🆔»." % (_кого, _назв, _текст[:100]))
                elif "chat_admin_required" in _низ or "is an administrator" in _низ or "chat admin" in _низ:
                    await update.message.reply_text(
                        "🤷 Этого человека забанить нельзя: он сам администратор «%s» (%s).\n"
                        "Сними с него админку в Telegram — тогда бан сработает." % (_назв, _текст[:100]))
                else:
                    await update.message.reply_text(
                        "⚠️ Не удалось %s %d в «%s»: %s\n"
                        "Проверить права: «права %s»." % (_акт, _кого, _назв, _текст[:200], _чат))
            return
        ...
        if _c324 and (getattr(update.effective_chat, "type", "") == "private" or _c324[0][0] >= 0.8):
            ...
            if _наш:
                ...
                if _ar or _tr:
                    ...
                    return
            # Книга не из нашей базы (Табарани, Байхаки и прочие вне 41 сборника) — текста у нас
            # физически нет. Честно говорим об этом, а не отделываемся ссылкой: владелец должен
            # понимать, ПОЧЕМУ хадиса нет, иначе это читается как отказ без причины.
            # #613: называем сам ЗАПРОС в тексте — в группе ответ приходил без видимого вопроса
            # и выглядел случайным (quote при пересылке теряется).
            _имя324 = _c324[0][2] if _c324 else ""
            await update.message.reply_text(
                f"📚 По запросу «{text}»: это «{_имя324}» из библиотеки Мактабы.\n"
                f"Текста хадисов этой книги в нашей базе нет — у нас 41 сборник, она в них не входит.\n"
                f"Открыть книгу на №{_n324} прямо в приложении:"
