...
ЗАКОН_ВЛАДЕЛЬЦА_РЕ = re.compile(
    ...
    r'запомни\s+(?:закон|правило|навсегда)|по\s+умолчанию\s+так|это\s+закон|'
    r'запомни\s+что|не\s+надо\s+в\s+таких\s+случаях|каждый\s+раз\s+\w+|'
    ...
    if _dsoc is not None:
        ...
        try:
            ...
            if _мп:
                ...
                if _записал_правило:
                    собрано += ("\n\n📌 Записал себе правило #%d — оно действует с этой минуты. "
                                "Технадзор проверит и либо закрепит навсегда, либо отменит."
                                % _записал_правило)
        except Exception:
            pass
                ...
                if _н2:
                    try:
                        await application.bot.send_message(
                            LOG_CHAT_ID,
                            '📌 <b>ЗАКОН ВЛАДЕЛЬЦА ПОЙМАН СТРАХОВКОЙ</b> — правило #%d, статус '
                            '«поймано» (НЕ действует, ждёт технадзора).\n<i>%s</i>\n\n'
                            'Помощник не записал его сам. Утвердить или отменить — технадзору.'
                            % (_н2, (_dsoc or '')[:600]), parse_mode='HTML')
                    except Exception:
                        pass
        except Exception:
            pass
