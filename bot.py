...
def _оценки_критиков(mid, rankH='', rankD=''):
    ...
    if not св:
        _з = []
        if rankH: _з.append("Ибн Хаджар: " + str(rankH))
        if rankD: _з.append("аз-Захаби: " + str(rankD))
        if not _з:
            return ""
        return ("<blockquote expandable>⚖️ Оценки критиков\n" + "\n".join(esc(x) for x in _з)
                + "\n\nПодробный разбор — в карточке приложения.</blockquote>")
    стр = []
    for з in св:
        кр = esc(str((з[0] if len(з) > 0 else '') or '').strip())
        ...
        if not слова:
            continue
        ст = "• <b>%s</b>: %s" % (кр, esc(слова[:300]))
        if цит and цит != слова:
            ст += " — <i>%s</i>" % esc(цит[:200])
        if кн:
            ст += " <code>%s</code>" % esc(кн[:60])
        стр.append(ст)
    if not стр:
        return ""
    ...
    БЮДЖЕТ, взято, ушло = 2600, [], 0
    for ст in стр:
        if ушло + len(ст) > БЮДЖЕТ and взято:
            break
        взято.append(ст); ушло += len(ст) + 1
    шапка = "⚖️ Что сказали о нём учёные-критики — свидетельств: %d" % len(стр)
    хвост = "\nПолный разбор со ссылками на лист книги — в карточке приложения."
    if len(взято) < len(стр):
        хвост = ("\nЗдесь показаны %d из %d — остальные %d не влезли в одно сообщение."
                 % (len(взято), len(стр), len(стр) - len(взято))) + хвост
    return "<blockquote expandable>" + шапка + "\n" + "\n".join(взято) + хвост + "</blockquote>"
...
async def narr_card_reply_text(query, ref):
    ...
    parts.append(f"📚 Мактаба: https://shamela.ws/index.php/author/{rid}")
    ...
    try:
        ...
        if _вики.get('u'):
            parts.append("🌍 Википедия: " + _вики['u'])
    except Exception:
        pass
