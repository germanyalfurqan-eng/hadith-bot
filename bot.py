    ...
    if _dsoc is not None:
        ...
        if _зовут_вникнуть:
            try:
                ...
                _н = getattr(update.message, 'message_id', 0)
                _окно = [x for x in (ЧАТ_ЛЕНТА.get(_ч) or []) if abs((x.get('i') or 0) - _н) <= 30]
                if _окно:
                    _разговор = chr(10).join('#%s %s: %s' % (x.get('i'), x.get('кто'),
                                                             (x.get('т') or '')[:400])
                                             for x in _окно[-40:])
                    реплики = list(реплики) + [{'role': 'system', 'content':
                        'РАЗГОВОР ВОКРУГ ЭТОГО СООБЩЕНИЯ (тебя попросили вникнуть в него). '
                        'Читай и отвечай ПО НЕМУ, а не проси повторить:' + chr(10) + _разговор}]
            except Exception:
                pass
        шапка = "💬 DSOC думает…"
