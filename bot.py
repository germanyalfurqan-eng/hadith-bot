...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _dsoc is not None:
        ...
        _публично = getattr(update.effective_chat, 'type', '') in ('group', 'supergroup', 'channel')
        if _публично:
            _к_проц = (_сист + _перепис) / DSOC_ОКНО * 100
            _хвост = ('<blockquote expandable>🧠 контекст %d из 1 000 000 (%.1f%%) · ⏱ %.1f с'
                      '</blockquote>' % (_сист + _перепис, _к_проц, сек))
        else:
...
async def _api_serve(application=None):
    ...
    КУХНЯ_РЕ = (
        ...
        (re.compile(r'(?<![\w/])/(?:home|Users|root|c)/[^\s"\'<>]+'), '<путь скрыт>'),
        (re.compile(r'\$\s?\d+[.,]\d+'), '<сумма скрыта>'),                 # деньги
        (re.compile(r'\b\d+[.,]?\d*\s?(?:МБ|ГБ|KB|MB|GB|КБ)\b'), '<вес скрыт>'),
        (re.compile(r'\b(?:вход|выход)\s+\d+\s*(?:ток)?\b'), ''),           # расход токенов
        (re.compile(r'\b(?:deepseek|gpt|groq|gemini|llama|qwen|opencode)[\w.\-]*\b', re.I),
         'модель'),
        (re.compile(r'\bBEARER\s+\S+|\b[A-Za-z0-9_\-]{32,}\b'), '<скрыто>'),  # ключи и токены
    )
