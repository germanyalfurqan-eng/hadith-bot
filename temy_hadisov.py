...
def тема(текст):
    # 🔴 06.09.2026. Разметка идёт 88 секунд на хадис; при 170 тысячах мест это месяцы работы.
    # Замер показал, где время: на длинном хадисе (2767 знаков) полный текст занял 437 с, а
    # последние 1200 знаков — 139 с, ВТРОЕ быстрее, и тема вышла не хуже, а полнее.
    # Почему хвост, а не начало: в начале стоит ИСНАД — цепочка имён, которую указание прямо
    # велит не пересказывать. Мы платили за чтение того, что просили не использовать.
    # Короткие тексты не режем: там резать нечего, а обрезка отняла бы половину дела.
    _для_модели = текст[-1200:] if len(текст) > 1400 else текст[:3000]
